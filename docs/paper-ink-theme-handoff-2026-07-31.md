...
>
> **已知且刻意保留：导出的 PDF 里粗体和正文一样粗。** `PdfFontSet` 的四个字重槽
> 指向同一份数据——这是**换字体之前就有的行为**，不是随包衬线带来的，原来用
> Noto Sans SC 时同样没有真粗体。要修只能再打包一份定死 w700 的静态实例，
> 实测 3.05MB，安装包总增量会从 5.19MB 变成 8.24MB。2026-08-22 权衡后决定不做：
> 这个限制存在已久且无人反馈，不值这个体积。真要加，`scripts/fonts/` 里
> `fonttools varLib.instancer <子集> wght=700` 一条命令就能产出。
>
> 两个坑，都已固化在代码注释里：
