...
- 2026-08-27: `dropped` 在可变刷新率（LTPO）屏幕上仍然可能虚高：固定的 8.3ms 预算做除数，
  ...
  不要用这个数**。
- 2026-08-28: **Sentry 的 iOS profile 不上传 dSYM 等于白开。** 采样、线程、栈深度都正常，
  Apple 系统库的帧有名字，我们自己二进制的帧全是 `unknown`。加 `profilesSampleRate` 的
  同一轮就该把符号上传一起做掉 —— 只开一半的观测手段比不开更浪费时间，因为它看起来像是
  开了。另外要知道这条路的上限：补符号后能读 Flutter 引擎（C++）和 Runner（ObjC），
  **Dart 帧大概率仍然打不开**（AOT 机器码不是靠 DWARF 描述的）。
- 2026-08-28: 记录页现在有**三条互相独立**的线，别再当成一个问题一起查：①冷启动首滑
  （大量挂载 + 挂载引出的帧外异步工作）②整列表重建（`depsΔ`/`dataΔ` 带出 `built=全部`，
  记忆化全中仍要 30+ms，钱在 sliver 层）③首屏光栅。热态滚动已经打平（`frameJank=0`）。
- 2026-08-28: 设备预算差一倍会把同一个缺陷藏起来。测试用的那台 iPad 是 60Hz（16.7ms），
  `worstBuild=36.2ms` 只掉 2 帧看不出来；同样的 36ms 在 120Hz（8.3ms）上掉 4~5 帧就是
  「卡」。**「在 iOS 上很流畅」不等于「iOS 上没这个问题」。**
- 2026-08-28: 观测手段的可用性要先对着**真正出问题的设备**核一遍。Sentry profiling 只有
  iOS/macOS 有，而卡的是 120Hz LTPO 的 Android —— 能 profile 的设备不卡，卡的设备不能
  profile。这一点在提议这条路线时就该想到，不该等拿到数据才发现。
