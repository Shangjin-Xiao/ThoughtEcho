...
**盲点:** `AnniversaryDigitGlyphs` 包含生成和计算周年数字 SVG 路径的核心纯函数，在测试环境中用于快速构建和校验周年蛋糕及数字显示格式。然而，它作为一个基础的测试工具集却没有自己的测试覆盖。如果后续调整了标签验证或补全规则，容易导致依赖它的上层组件测试集遭遇大面积的断言失败。
**对策:** 为其编写独立的纯函数单元测试，涵盖了正常的路径生成、无效输入回退、尺寸计算以及 SVG 数值格式化等边界条件的验证。这保证了底层辅助工具自身的健壮性。
