...
## 2024-05-18 - [补全多语言词条 addAiService]
**发现:**  `app_de.arb`, `app_es.arb`, `app_fr.arb`, `app_ja.arb`, 和 `app_ko.arb` 中缺失大量键。之前的批量处理脚本因为沙盒环境无法连接外部翻译API而失败。代码审查指出直接拷贝英文会导致用户体验不佳。
**规则:** 翻译缺失的多语言词条时，应当优先翻译一个或少量具有代表性的词条，并在脚本中直接提供翻译内容，或者使用内部可以运行的翻译工具，而不是简单地复制回退的英语内容。
