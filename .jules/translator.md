...
**发现:** `lib/l10n/app_en.arb` 中缺失了 `zh.arb` 里新增的 `@` 开头的 meta 键（如 `@generatingInsightsForRange`），导致 `flutter gen-l10n` 遇到无法识别的 placeholders 或缺失的类型。
**规则:** 凡是在 `zh.arb` (源文件) 中定义了带 placeholder 的词条，在其它语言（特别是 `en.arb` 等主要语言包）中也必须同步包含对应的 `@key` meta 块，以确保占位符类型能被正确解析。
