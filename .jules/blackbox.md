...
## 2024-05-24 - [完善 ThoughtEchoDiscoveryService 结构化日志]
**异常:** [设备发现服务在解析组播消息、发送套接字以及创建设备公告时，异常仅通过 debugPrint 打印，缺乏结构化日志和生产环境记录]
**拦截:** [使用 logError 记录异常，包含 stackTrace 和 source，保留了原有的 debugPrint 并在开发环境保留原始数据打印，确保无敏感数据上报]
