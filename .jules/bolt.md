...
**Action:** 在 `lib/widgets/note_list/note_list_items.dart` 的 `_selectSameMonthNotes` 和 `_selectSameCategoryNotes` 方法中，将 `for (final id in _selectedExportNoteIds)` 内部的 `_quotes.firstWhereOrNull` 查找逻辑，替换为先遍历 `_quotes` 收集选中笔记的月份/分类，再遍历 `_quotes` 按收集结果筛选，并使用 `_selectedExportNoteIds.contains(quote.id)`，避免 O(M*N) 的嵌套线性查找，将整体时间复杂度降低为 O(N)。
