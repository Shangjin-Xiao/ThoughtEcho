...
**Learning:**
在清理包含大量文件的临时目录（如 `temp_media`）时，对每个文件逐个调用 `await entity.delete()` 会在 Dart 事件循环和文件系统 Native API 间产生极高的异步 I/O 开销。使用整目录递归删除 `tempMediaDir.delete(recursive: true)` 配合 `tempMediaDir.create(recursive: true)` 重新建空的根目录，可以将全部文件删除合并为单次底层 OS unlink 目录树调用。同时保留个别删除作为 try-catch 降级方案，以提升强容错性。
