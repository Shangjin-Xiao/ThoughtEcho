...
def generate_markdown_report(results, output_path):
    ...
    for item in results:
        ...
        if item["id"] == "scenario_1_article_generation":
            ...
            md.append("- **防串味隔离性验证**：面对同一句日常闲聊“今天天气不错，泡了一杯喝的，坐下来发呆”，阿澈得到的是关于咖啡风味、窗外发呆与片刻清空的默契陪伴；林晚得到的是关于温润茶汤、案头图纸暂歇与慢节奏的温柔回应。**阿澈回复中未检测到林晚专有标记，林晚回复中未检测到阿澈专有标记。**")
        md.append("")
    ...
    md.append(f"| **跨画像串味标记数 (Bleeding Count)** | N/A | {total_a_bleeding} 处命中 | {total_l_bleeding} 处命中 | 提示词层面无交叉串味 |")
