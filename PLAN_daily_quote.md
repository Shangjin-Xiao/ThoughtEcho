...
### 9.14 最终确定的多语言 API 接入清单与实施计划（更新版）

基于 API 稳定性、连通性及内容质量的测试，最终确定采用“用户自选 API + 底层国际化标签兜底”的策略。

#### 9.14.1 目标 API 矩阵
| 标识符 | 语言 | 界面显示名称 | 接口特点说明 |
| :--- | :--- | :--- | :--- |
| `hitokoto` | 🇨🇳 中文 | **一言 (Hitokoto)** | *(默认)* 最经典的中文短句接口，支持动画、文学等细分类别。 |
| `zenquotes` | 🇬🇧 英文 | **ZenQuotes** | 提供高质量人工精选英文名言，内容深刻。 |
| `api_ninjas` | 🇬🇧 英文 | **API Ninjas** | 支持多达 50+ 种细分英文类型（需免费 Key）。 |
| `meigen` | 🇯🇵 日文 | **名言教えるよ** | 纯正的日文名言警句接口，无分类。 |
| `kadvice` | 🇰🇷 韩文 | **Korean Advice** | 优质的韩文名言与人生建议，无分类。 |

#### 9.14.2 核心 UI 与交互落地逻辑

**1. 设置页面的动态展示（分类防爆炸设计）**
- **现有中文一言**：保持现有的平铺界面不变。
- **切换 API 入口**：放置在设置页面最下方。
- **海量外语分类处理**：对于像 API Ninjas 这样有上百个分类的 API，不再使用平铺界面。而是提供一个入口点击进入一个**全新的分类选择页**。
  - 顶部带**搜索功能**。
  - 默认情况下**全选**所有类别（并在已选池中随机抽取）。
  - 提供**一键全部清除**按钮。
  - **边界兜底**：如果用户在这个新界面“全不选”就退出了，程序在请求网络时自动视为“无视分类限制，全局随机”。

**2. 双击保存与标签逻辑（防止标签污染）**
- **底层统一**：不管用哪个 API，所有名言都在后台被格式化为统一的数据结构。
- **主标签严格国际化**：双击名言弹出编辑器时，程序**永远且仅**为其打上底层的“主标签暗号”。该标签在不同系统语言下自动翻译（中文系统显示 `#每日一言`，英文系统显示 `#Daily Quote`）。
- **细分标签克制化**：如果当前用的是 Hitokoto，程序依然会自动追加 `#动画` 等 12 个细分类别；如果用的是**其他外语 API**，程序将**不再自动添加**任何原生的细分英文标签（如 `#success`），以此彻底避免外语标签大量污染用户的标签库。

**3. 本地笔记与无网回退的展示逻辑重构（关键）**
无论是用户主动开启了“仅使用本地笔记”，还是遭遇了断网/API崩溃的“被动兜底”，统一走以下逻辑：
- **新增 UI 选项**：在设置里增加明确的选项，并加上文案提示：“*无网络连接时，也将使用此规则展示本地笔记*”。
  - 选项 A：只展示带固定标签（#每日一言）的旧笔记。
  - 选项 B：从全部本地笔记中随机展示。
- **数据层解耦**：彻底废弃以前通过强制文字匹配 `'每日一言'` 来寻找兜底笔记的做法。必须改为使用底层固定的 `DatabaseService.defaultCategoryIdHitokoto` 或对应固定 Tag ID 进行查找。因为该标签将全面支持国际化，强匹配中文字符串会引发非中文环境下的查询 Bug。
