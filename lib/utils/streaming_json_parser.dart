...
class StreamingJsonParser {
  ...
  static Future<Map<String, dynamic>> parseJsonFile(
    ...
    try {
      ...
      logDebug('开始流式解析JSON文件: ${file.path} (${(fileSize / 1024 / 1024).toStringAsFixed(1)}MB)');
  ...
  static Future<Map<String, dynamic>> _parseJsonStreaming(
    ...
    await for (final chunk in stream) {
      ...
      // 定期检查内存压力
      if (bytesRead % (10 * 1024 * 1024) == 0) { // 每10MB检查一次
  ...
  static Future<bool> canSafelyParse(File file) async {
    try {
      ...
      if (fileSize > maxSafeSize) {
        logDebug('JSON文件过大，无法安全解析: ${(fileSize / 1024 / 1024).toStringAsFixed(1)}MB');
        return false;
      }
