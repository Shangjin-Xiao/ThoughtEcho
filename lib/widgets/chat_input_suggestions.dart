...
class ChatInputSuggestions extends StatelessWidget {
  ...
  static List<String> generateSuggestions(
    ...
    // 检测关键词并提供相关建议。中英文关键词同时匹配，与笔记内容和界面语言
    // 无关，避免英文用户因关键词不匹配而收不到针对性建议。
    final lowerContent = noteContent.toLowerCase();

    if (lowerContent.contains('问题') ||
        lowerContent.contains('困难') ||
    ...
    if (lowerContent.contains('学习') ||
        lowerContent.contains('知识') ||
    ...
    if (lowerContent.contains('计划') ||
        lowerContent.contains('目标') ||
    ...
    if (lowerContent.contains('感受') ||
        lowerContent.contains('情感') ||
