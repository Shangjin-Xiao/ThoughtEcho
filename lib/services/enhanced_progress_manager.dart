...
class EnhancedProgressManager {
  static final EnhancedProgressManager _instance = EnhancedProgressManager._internal();
  ...
  final StreamController<ProgressEvent> _progressController = StreamController<ProgressEvent>.broadcast();
  ...
  String startOperation({
    ...
    _emitProgressEvent(ProgressEvent(
  ...
  /// 操作失败
  void failOperation(String operationId, String error, {StackTrace? stackTrace}) {
    final progress = _activeOperations[operationId];
    if (progress == null) return;
  ...
  List<OperationProgress> getActiveOperations() {
    return _activeOperations.values.where((op) => !op.isCompleted && !op.isCancelled && !op.isFailed).toList();
  }
  ...
  void _calculateSpeed(OperationProgress progress) {
    ...
    if (elapsed > 0 && progress.currentSize > 0) {
      ...
      if (recentSamples.isNotEmpty) {
        final totalBytes = recentSamples.fold<int>(0, (sum, sample) => sum + sample.bytes);
        final totalTime = recentSamples.fold<int>(0, (sum, sample) => sum + sample.intervalMs);
      ...
      // 添加新的速度样本
      progress.speedSamples.add(SpeedSample(
        timestamp: now,
