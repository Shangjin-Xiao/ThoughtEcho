...
class StartupMonitorService {
  ...
  /// 记录启动步骤
  static Future<void> recordStep(String step, {Map<String, dynamic>? data}) async {
    if (!_isMonitoring) return;
    ...
    if (_startupTime != null) {
      stepData['elapsed_ms'] = DateTime.now().difference(_startupTime!).inMilliseconds;
    }
  ...
  /// 记录监控数据
  static Future<void> _logMonitorData(String event, Map<String, dynamic> data) async {
    try {
      final logEntry = {
        'event': event,
        'timestamp': DateTime.now().toIso8601String(),
  ...
  static Future<void> _generateStartupReport() async {
    try {
      ...
      for (final data in _performanceData) {
        buffer.writeln('${data['timestamp']} [${data['event']}] ${data['data']}');
      }
  ...
  static Future<void> _generateEmergencyReport() async {
    try {
      ...
      final emergencyFile = File(path.join(desktop.path, 'ThoughtEcho_启动超时报告_$timestamp.txt'));
  ...
  static Future<void> _createDesktopReport(String content) async {
    try {
      ...
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final desktopFile = File(path.join(desktop.path, 'ThoughtEcho_启动报告_$timestamp.txt'));
