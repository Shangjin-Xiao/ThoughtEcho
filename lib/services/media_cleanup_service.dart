...
class MediaCleanupService {
  ...
  static Future<Map<String, int>> performPeriodicCleanup() async {
    try {
      ...
      // 1. 清理过期临时文件
      final expiredTempFiles = await TemporaryMediaService.cleanupExpiredTemporaryFiles();
  ...
  static Future<Map<String, dynamic>> performFullCleanup({
    ...
    try {
      ...
      final tempFiles = dryRun 
          ? (await TemporaryMediaService.getTemporaryFilesStats())['totalFiles'] as int
          : await TemporaryMediaService.cleanupAllTemporaryFiles();
      results['tempFilesCleared'] = tempFiles;
      
      // 3. 清理孤儿媒体文件
      final orphanFiles = await MediaReferenceService.cleanupOrphanFiles(dryRun: dryRun);
      ...
      if (!dryRun) {
        ...
        // 计算节省的空间
        final spaceSaved = beforeStats['totalSizeMB'] - afterStats['totalSizeMB'];
  ...
  static Future<Map<String, dynamic>> verifyMediaIntegrity() async {
    try {
      ...
      for (final quote in quotes) {
        final mediaPaths = await MediaReferenceService.extractMediaPathsFromQuote(quote);
