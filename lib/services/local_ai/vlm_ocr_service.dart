...
class VLMOCRService extends ChangeNotifier {
  ...
  Future<OCRResult> recognizeFromFile(
    ...
    try {
      _status =
          const VLMOCRStatus(state: VLMOCRState.processing, progress: 0.0);
      notifyListeners();
      ...
      logInfo('开始 VLM OCR 识别: $processedImagePath, 模式: $mode',
          source: 'VLMOCRService');
      ...
      logInfo('VLM OCR 识别完成: ${recognizedText.length} 字符',
          source: 'VLMOCRService');
      return result;
    } catch (e) {
...
请输出识别的文字：''';
  ...
  Future<String> _fallbackSimulation(String imagePath, String prompt) async {
    ...
    // 如果没有真实模型，返回空字符串或抛出异常，而不是返回占位文本。
    // 这允许上层逻辑（如 HybridOCRService）正确处理回退或错误。
    throw Exception('vlm_model_missing_no_simulation');
