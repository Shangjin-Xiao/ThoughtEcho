...
/// Helper class for media reference cleanup operations
class _MediaReferenceCleanup {
  /// 检测孤儿文件（没有被任何笔记引用的文件）
  ...
  static Future<_CleanupPlan> _planOrphanCleanup() async {
    ...
    for (final filePath in allMediaFiles) {
      final normalizedPath = await MediaReferenceService._normalizeFilePath(
        filePath,
  ...
  static Future<Map<String, Map<String, Set<String>>>>
    ...
    final rows = await db.query(MediaReferenceService._tableName,
  ...
  static Future<Map<String, Map<String, Set<String>>>>
    ...
    for (final quote in quotes) {
      ...
      final mediaPaths = await MediaReferenceService.extractMediaPathsFromQuote(
        quote,
        cachedAppPath: appPath,
      );
