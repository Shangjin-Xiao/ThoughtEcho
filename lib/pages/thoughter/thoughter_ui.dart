...
extension _ThoughterUI on _ThoughterPageState {
  ...
  /// 左右留白维持在 [_kComposerMarginFocused] 那一档——键盘没了，框还撑得
  /// 宽宽的贴着屏幕两边缩不回去，描边也一直亮着。焦点留着还有第二个后果：
  /// 下一次别处的浮层关掉时，系统会把键盘原样还回来。
  ///
  ...
  bool _shouldKeepAliveMessage(app_chat.ChatMessage message) {
    ...
    if (type == NoteProposalArtifact.typeName) return true;
    if (type == 'ask_user') {
      return message.id == _pendingAskUserMessageId;
    }
    return false;
  }
}
