...
class _OnboardingPageState extends State<OnboardingPage> {
  ...
  int _selectedStartPage =
      AppSettings.defaultSettings()
          .defaultStartPage; // 使用 AppSettings.defaultSettings() 获取默认值
    ...
    if (mounted) {
      setState(() {
        ...
        _isLocationPermissionEnabled =
            ...
            _locationPermissionStatus.isLimited;
      });
    }
  }
  ...
  Future<void> _finishOnboarding() async {
    ...
    try {
      ...
      final mmkvService = Provider.of<MMKVService>(
        context,
        listen: false,
      ); // 获取 MMKV 服务
      ...
      String lastRunBuildNumberString =
          mmkvService.getString(mmkvKeyLastRunVersion) ?? '0';
      ...
      bool isUpdateRequiringMigration =
          currentBuildNumber > lastRunBuildNumber &&
          currentBuildNumber >= migrationNeededFromBuildNumber;
      bool needsMigration = isFirstSetup || isUpdateRequiringMigration;

      logDebug(
        '版本检查: 当前版本=$currentBuildNumber, 上次运行版本=$lastRunBuildNumber, 是否首次设置=$isFirstSetup, 是否需要迁移=$needsMigration',
      );

      try {
        // 确保数据库已初始化
        await databaseService.init();
        logDebug('数据库初始化完成 (引导流程)');

        // 添加此行: 明确初始化默认一言分类标签
        await databaseService.initDefaultHitokotoCategories();
        logDebug('引导流程中已显式初始化默认一言分类标签');

        if (needsMigration) {
          logDebug('开始执行引导流程中的数据迁移...');
          try {
            // 确保数据库已完全初始化后再执行迁移
            ...
            logDebug('数据迁移成功完成，已标记迁移状态');
          } catch (e, stackTrace) {
            logDebug('引导流程中数据迁移失败: $e');
            logError(
              '引导流程数据迁移失败',
              error: e,
            ...
            if (isFirstSetup) {
              ...
              await settingsService.setDatabaseMigrationComplete(true);
              logDebug('首次设置：即使迁移失败也标记完成，避免阻塞新用户');
            }

            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('数据格式更新时遇到问题'),
        ...
        if (currentBuildNumber > lastRunBuildNumber) {
          await mmkvService.setString(
            mmkvKeyLastRunVersion,
            currentBuildNumberString,
          );
          logDebug('已更新上次运行版本号记录为: $currentBuildNumberString');
        }
      } catch (e, stackTrace) {
        logDebug('引导流程中数据库初始化失败: $e');
        logError(
          '引导流程数据库初始化失败',
          error: e,
          stackTrace: stackTrace,
  ...
  Future<void> _saveSettings() async {
    try {
      ...
      clipboardService.setEnableClipboardMonitoring(
        _isClipboardMonitoringEnabled,
      );
          ...
          child: Stack(
            children: [
              ...
              PageView(
                ...
                physics:
                    widget.showUpdateReady && !widget.showFullOnboarding
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      _currentPage > 0
                          ? TextButton.icon(
                            onPressed:
                                _isFinishing ? null : _previousPage, // 禁用按钮
                      ...
                      _currentPage < 3
                          ...
                          : FilledButton.icon(
                            onPressed:
                                _isFinishing ? null : _finishOnboarding, // 禁用按钮
                            icon:
                                _isFinishing
                                    ? const SizedBox(
                                      width: 18,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('为了提供完整体验，心迹需要以下权限：', style: theme.textTheme.bodyLarge),
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text('• 位置信息仅用于获取天气和记录笔记位置', style: theme.textTheme.bodyMedium),
              ],
            ),
          ),
        ],
  ...
  String _getLocationSubtitle() {
    switch (_locationPermissionStatus) {
      ...
      case PermissionStatus
          .limited: // Limited access is also considered enabled
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text('选择您感兴趣的内容类型：', style: theme.textTheme.bodyLarge),
          const SizedBox(height: 20),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text('选择您希望打开应用时首先看到的页面：', style: theme.textTheme.bodyLarge),
          const SizedBox(height: 24),
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                children:
                    startPageOptions.entries.map((entry) {
