...
import 'package:thoughtecho/services/agent_tools/complete_task_tool.dart';
...
void main() {
  ...
  group('AgentService native tool loop', () {
    test('requires explicit completion even when first response is plain text',
        () async {
      ...
      final responses = <openai.ChatCompletion>[
        _textCompletion('先说一句就停。'),
        _toolCallCompletion(
          callId: 'call_complete',
          toolName: 'complete_task',
          args: const {'result': '任务已明确完成。'},
        ),
      ];

      final service = AgentService(
        settingsService: settings,
        tools: const [CompleteTaskTool()],
      ...
      final response = await service.runAgent(userMessage: '完成这个任务');

      expect(response.content, '任务已明确完成。');
    });

    test('defers completion when another tool is requested in the same turn',
        () async {
      final provider = const AIProviderSettings(
        id: 'openai',
        name: 'OpenAI',
      ...
      final responses = <openai.ChatCompletion>[
        _multiToolCallCompletion([
          _buildToolCall(
            callId: 'call_complete_early',
            toolName: 'complete_task',
            args: const {'result': '过早完成'},
          ),
          _buildToolCall(
            callId: 'call_proposal',
            toolName: 'propose_edit',
            args: const {'query': 'draft'},
          ),
        ]),
        _toolCallCompletion(
          callId: 'call_complete_final',
          toolName: 'complete_task',
          args: const {'result': '建议已经准备好。'},
        ),
      ];

      final service = AgentService(
        settingsService: settings,
        tools: [const CompleteTaskTool(), proposal],
        apiKeyResolver: (_) async => 'test-key',
        completionRequester: ({
          required provider,
          required messages,
      ...
      final response = await service.runAgent(userMessage: '准备修改建议');

      expect(proposal.executeCount, 1);
      expect(response.content, '建议已经准备好。');
      expect(
          response.toolCalls.any((call) => call.id == 'call_proposal'), isTrue);
      expect(
        response.toolCalls.any((call) => call.id == 'call_complete_early'),
        isFalse,
      );
    });

    test('requires explicit completion after tools when completion tool exists',
        () async {
      final provider = const AIProviderSettings(
        id: 'openai',
        name: 'OpenAI',
      ...
      final settings = _FakeSettingsService(provider);
      final searchTool = _CountingTool(
      ...
      final responses = <openai.ChatCompletion>[
        ...
        _textCompletion('我查完了。'),
        _toolCallCompletion(
          callId: 'call_complete',
          toolName: 'complete_task',
          args: const {'result': '已完成检索，没有找到相关笔记。'},
        ),
      ];
      ...
      final service = AgentService(
        settingsService: settings,
        tools: [searchTool, const CompleteTaskTool()],
        apiKeyResolver: (_) async => 'test-key',
        completionRequester: ({
          required provider,
          required messages,
          ...
          requestCount++;
          if (requestCount == 3) {
            completionTranscript = messages.map(_chatMessageText).join('\n');
          }
          return responses.removeAt(0);
        },
      );

      final response = await service.runAgent(userMessage: '查找今天的笔记');

      expect(requestCount, 3);
      expect(completionTranscript, contains('尚未通过 complete_task'));
      expect(response.content, '已完成检索，没有找到相关笔记。');
    });
