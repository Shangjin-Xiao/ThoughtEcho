...
void main() {
  group('大文件处理测试', () {
    ...
    test('LargeFileManager 应该正确处理边界情况', () async {
      ...
      try {
        final canProcess = await LargeFileManager.canProcessFile(emptyFile.path);
        expect(canProcess, false); // 空文件应该不能处理
        
        final fileSize = await LargeFileManager.getFileSizeSecurely(emptyFile.path);
        expect(fileSize, 0);
      } finally {
      ...
      final canProcessNonExistent = await LargeFileManager.canProcessFile(nonExistentPath);
      expect(canProcessNonExistent, false);
      
      final nonExistentSize = await LargeFileManager.getFileSizeSecurely(nonExistentPath);
    ...
    test('LargeVideoHandler 应该能够安全导入视频文件', () async {
      ...
      final content = Uint8List.fromList([...mp4Header, ...List.generate(1024 * 1024, (i) => i % 256)]); // 1MB+
    ...
    test('LargeVideoHandler 应该正确处理错误情况', () async {
      ...
      // 测试获取不存在文件的信息
      final videoInfo = await LargeVideoHandler.getVideoFileInfo('/fake/video.mp4');
    ...
    test('LargeVideoHandler 应该正确处理取消操作', () async {
      ...
      final content = Uint8List.fromList([...mp4Header, ...List.generate(5 * 1024 * 1024, (i) => i % 256)]); // 5MB+
      await sourceVideo.writeAsBytes(content);
