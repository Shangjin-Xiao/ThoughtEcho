...
void main() {
  group('Quote attribution classification & identity heuristics', () {
    ...
    group('isSelfAttributed, isExcerpt, isOriginal & resolveAttributionKind',
      ...
      test('英文标题如 Alice Notes 配合 alias Alice 能够大小写不敏感匹配为原创', () {
        final noteAlice = Quote(
          content: 'Rainy afternoon coffee thoughts.',
          date: '2026-08-20',
          source: 'Alice Notes',
        );
        expect(noteAlice.isSelfAttributed(userAliases: ['alice']), isTrue);
        expect(noteAlice.isExcerpt(userAliases: ['alice']), isFalse);

        final noteAliceWork = Quote(
          content: 'Rainy afternoon coffee thoughts.',
          date: '2026-08-20',
          sourceWork: 'Alice Notes',
        );
        expect(noteAliceWork.isSelfAttributed(userAliases: ['alice']), isTrue);
        expect(noteAliceWork.isExcerpt(userAliases: ['alice']), isFalse);
      });
