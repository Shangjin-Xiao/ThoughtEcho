  ...
  group('MediaOptimizationUtils pure functions test', () {
    group('getMimeType', () {
      test('should return correct MIME types for known extensions', () {
        ...
        expect(MediaOptimizationUtils.getMimeType('video.mov'), 'video/quicktime');
        expect(MediaOptimizationUtils.getMimeType('video.avi'), 'video/x-msvideo');
      ...
      test('should return null for empty string or no extension', () {
        ...
        expect(MediaOptimizationUtils.getMimeType('filename_without_extension'), isNull);
      });
    });

    group('estimateOptimizedSize', () {
      test('should estimate correct size for images', () {
        expect(MediaOptimizationUtils.estimateOptimizedSize(1000, 'image'), 300);
        expect(MediaOptimizationUtils.estimateOptimizedSize(1000, 'IMAGE'), 300);
      });

      test('should estimate correct size for videos', () {
        expect(MediaOptimizationUtils.estimateOptimizedSize(1000, 'video'), 700);
        expect(MediaOptimizationUtils.estimateOptimizedSize(1000, 'VIDEO'), 700);
      });

      test('should estimate correct size for audio', () {
        expect(MediaOptimizationUtils.estimateOptimizedSize(1000, 'audio'), 800);
        expect(MediaOptimizationUtils.estimateOptimizedSize(1000, 'AUDIO'), 800);
      });

      test('should return original size for unknown file types', () {
        expect(MediaOptimizationUtils.estimateOptimizedSize(1000, 'document'), 1000);
        expect(MediaOptimizationUtils.estimateOptimizedSize(1000, 'unknown'), 1000);
      });
