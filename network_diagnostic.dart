...
void main() async {
  ...
  try {
    ...
    for (int port = defaultPort + 1; port <= defaultPort + 5; port++) {
      try {
        ...
        AppLogger.i('  ✓ 成功绑定到替代端口 ${server.port}',
  ...
  try {
    ...
    AppLogger.i('  ✓ 成功创建UDP套接字，端口: ${socket.port}',
        source: 'NetworkDiagnostic');
    ...
    try {
      ...
      for (final interface in interfaces) {
        try {
          ...
          AppLogger.e('  ❌ 加入组播组失败 (接口: ${interface.name}): $e',
              source: 'NetworkDiagnostic');
        }
      }
      ...
      final sent = socket.send(messageBytes,
  ...
  try {
    ...
    try {
      final request =
          await client.get('127.0.0.1', port, '/api/localsend/v2/info');
      ...
      if (response.statusCode == 200) {
        ...
        AppLogger.e('  ❌ HTTP响应错误: ${response.statusCode}',
            source: 'NetworkDiagnostic');
      }
    } finally {
      client.close();
