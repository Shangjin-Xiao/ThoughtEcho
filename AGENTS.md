...
> **环境与设备提示**：若本地存在 `docs/DEVICE_INFO.md`（由 `.gitignore` 忽略），进入开发前请先阅读并严格遵守其设备资源与操作约束。
...
- 文件名要指向它现在是什么，不是曾经是什么。禁止 `_clean`、`_new`、`_v2`、`_fixed`、
  `_optimized` 这类状态词后缀，也不要留只做转发的 `_wrapper` 页面。改名一次改到底：文件、
  类、`part` 路径、目录、对应测试一起改。废弃的页面/服务直接删，不要留"已弃用"注释躺在
  `lib/` 里 —— 注释拦不住检索。
...
- UI 层禁止硬编码任何用户可见文本，包括按钮、菜单、Tooltip、Dialog、SnackBar、空状态和
  无障碍标签。
- 新文案与描述只需新增中英文（`lib/l10n/app_zh.arb` 与 `lib/l10n/app_en.arb`），占位符声明元数据，其他语言后续补充；未提供本地化的语言默认回退显示为英文（`l10n.yaml` 中首选语言为 `en`）。无需手动运行 `gen-l10n`（构建与 CI 会自动生成）；禁止手动编辑 `lib/gen_l10n/`。
...
- 🔒 **不可逆写入前先把整列快照到 `<列名>_backup`**，并在修复自己那条 UPDATE 里写原值
  ...
  **遗留列清理必须排在所有建快照列的迁移之前**：`_removeTagIdsColumn` 重建 `quotes` 用的是
  ...
  - 这条规则一度被 `migrateWeatherToKey` / `migrateDayPeriodToKey` 违反着：它们的 UPDATE
    只写规范化值，只靠 `_ensureBackupColumn` 建列那一次快照兜着，第二轮跑的时候原值就没了。
    已在 PR #538 补齐。参考实现看 `repairOutOfDomainSentiment`。
...
- `categories` 表、`category_id` 列和备份 JSON 的 `categories` 键存的是**标签**
  ...
  注意 API Ninjas 名言分类和 emoji 分组是真的 `category`，与标签无关。
